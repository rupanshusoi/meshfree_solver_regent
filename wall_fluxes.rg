...
do
	...
	for i = 0, 20 do
		...
		if itm == 0 then
			...
			for i = 0, 4 do
				qtilde_i[i] = pnbh[idx].q[i] - 0.5 * (delx * pnbh[idx].dq0[i] + dely * pnbh[idx].dq1[i])
			...
			for i = 0, 4 do
				...
				qtilde_k[i] = pnbh[itm].q[i] - 0.5 * phi_k[i] * (delx * pnbh[itm].dq0[i] + dely * pnbh[itm].dq1[i])
			end
