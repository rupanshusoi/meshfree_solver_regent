...
__demand(__inline, __cuda)
task outer_dGx_pos(compact : region(ispace(int1d), Point), idx : int, pmap : region(ispace(int1d), int), config : Config)
where 
  reads(compact.{x, y, nx,ny, xpos_conn, q, dq0, dq1, minq, maxq, min_dist}, pmap)
do
  var power : int = 0
...
__demand(__inline, __cuda)
task outer_dGx_neg(compact : region(ispace(int1d), Point), idx : int, pmap : region(ispace(int1d), int), config : Config)
where 
  reads(compact.{x, y, nx,ny, xneg_conn, q, dq0, dq1, minq, maxq, min_dist}, pmap)
do
  var power : int = 0
  var limiter_flag : int = 1
...
__demand(__inline, __cuda)
task outer_dGy_pos(compact : region(ispace(int1d), Point), idx : int, pmap : region(ispace(int1d), int), config : Config)
where 
  reads(compact.{x, y, nx,ny, ypos_conn, q, dq0, dq1, minq, maxq, min_dist}, pmap)
do
  var power : int = 0
  var limiter_flag : int = 1
